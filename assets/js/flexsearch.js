...
//
// Project override of themes/hextra/assets/js/flexsearch.js. Two deviations:
//  1) Hangul runs are encoded as overlapping bi/tri-grams instead of single
//     codepoints. Single-codepoint tokens make Korean queries match every
//     page that happens to share a syllable — n-grams move matching closer
//     to morpheme level.
//  2) search() uses suggest:false so all query tokens must hit (AND), not
//     just some (OR). Pairs well with n-gram tokens to cut noise.
...
(function () {
  ...
  // Non-Hangul tokens: other CJK glyphs stay one-char (no morpheme boundary
  // signal available without a real analyzer), Latin/Cyrillic/etc. stay
  // word-level. Hangul (AC00-D7A3) is intentionally absent here — handled
  // separately below.
  const otherTokenRegex = /[\u{4E00}-\u{9FFF}\u{3040}-\u{309F}\u{30A0}-\u{30FF}\u{3400}-\u{4DBF}\u{20000}-\u{2A6DF}\u{2A700}-\u{2B73F}\u{2B740}-\u{2B81F}\u{2B820}-\u{2CEAF}\u{2CEB0}-\u{2EBEF}\u{30000}-\u{3134F}\u{31350}-\u{323AF}\u{2EBF0}-\u{2EE5F}\u{F900}-\u{FAFF}\u{2F800}-\u{2FA1F}]|[0-9A-Za-zа-яÀ-ſЀ-ӿ؀-ۿঀ-৿Ḁ-ỿ֐-׿]+/gu;
  const hangulRunRegex = /[\u{AC00}-\u{D7A3}]+/gu;

  // Same encode runs at both index time and query time. Keeping that
  // invariant is what makes n-gram matching work without server-side prep.
  function encode(str) {
    str = ('' + str).toLowerCase();
    const tokens = [];

    const others = str.match(otherTokenRegex);
    if (others) tokens.push(...others);

    const hangulRuns = str.match(hangulRunRegex);
    if (hangulRuns) {
      for (const run of hangulRuns) {
        ...
        // Overlapping bigrams: "패스워드" -> 패스, 스워, 워드
        for (let i = 0; i + 2 <= run.length; i++) {
          tokens.push(run.slice(i, i + 2));
        }
        // Overlapping trigrams: "패스워드" -> 패스워, 스워드
  ...
  function performSearch(query) {
    ...
    // suggest:false → AND match (all query tokens must hit). Combined with
    // n-gram encoding above, this gives Korean queries near-word precision.
    const pageResults = pageIndex.search(query, maxPageResults, { enrich: true, suggest: false })[0]?.result || [];
    ...
    for (let i = 0; i < pageResults.length; i++) {
      ...
      const sectionResults = sectionIndex.search(query,
        { enrich: true, suggest: false, tag: { 'pageId': `page_${result.id}` } })[0]?.result || [];
      const occurred = {};
      const group = {
        _page_rk: i,
        route: '',
